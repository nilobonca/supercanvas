    ...
    <aside className="w-80 h-full bg-[#FAF9F6] dark:bg-[#111115] border-l border-stone-200/90 dark:border-white/10 flex flex-col select-none shrink-0 z-20 text-stone-900 dark:text-neutral-100 animate-in slide-in-from-right-4 duration-150">
      ...
      <div className="p-2 border-b border-stone-200/90 dark:border-white/10 flex items-center justify-between bg-white/70 dark:bg-white/[0.02]">
        ...
        <div className="flex items-center gap-1 bg-stone-100/80 dark:bg-white/5 p-0.5 rounded-lg border border-stone-200/80 dark:border-white/5 text-xs">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md font-medium transition-colors cursor-pointer ${
              detailsSidebarTab === 'properties'
                ? 'bg-white dark:bg-white/15 text-purple-700 dark:text-purple-300 shadow-xs'
          ...
          >
            ...
            {totalPropsCount > 0 && (
              <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 font-mono">
                {totalPropsCount}
              </span>
            )}
          </button>
      ...
      <div className="px-3.5 py-2 border-b border-stone-200/80 dark:border-white/5 bg-stone-100/50 dark:bg-white/[0.02] text-xs text-stone-500 dark:text-neutral-400 flex items-center justify-between truncate">
        <span className="truncate">
          Nota: <span className="text-purple-700 dark:text-purple-300 font-medium">"{currentDocTitle || 'Sem nota ativa'}"</span>
        </span>
      </div>
          ...
          {backlinks.length === 0 ? (
            <div className="py-16 text-center text-stone-400 dark:text-neutral-500 text-xs flex flex-col items-center gap-2">
              ...
              <span className="text-[11px] text-stone-400 dark:text-neutral-500 max-w-[220px] leading-relaxed">
                Digite <code className="bg-stone-100 dark:bg-white/10 px-1 py-0.5 rounded border border-stone-200 dark:border-white/10 text-purple-700 dark:text-purple-300 font-mono">[[{currentDocTitle}]]</code> em outra nota para conectá-las!
              </span>
            </div>
          ) : (
            backlinks.map((link, idx) => (
              <div
                ...
                className="p-2.5 rounded-lg bg-white dark:bg-white/5 border border-stone-200/90 dark:border-white/10 hover:border-purple-300 dark:hover:border-purple-500/40 hover:bg-stone-50 dark:hover:bg-white/[0.08] shadow-xs cursor-pointer transition-all group"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5 truncate">
                    <FileText className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400 shrink-0" />
                    <span className="text-xs font-medium text-stone-800 dark:text-neutral-200 group-hover:text-purple-700 dark:group-hover:text-purple-300 truncate">
                      {link.sourceTitle}
                    </span>
                  </div>
                  <ArrowRight className="w-3 h-3 text-stone-400 group-hover:text-purple-600 dark:group-hover:text-purple-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
