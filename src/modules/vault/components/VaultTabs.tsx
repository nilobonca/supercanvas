    ...
    <div className="flex items-center bg-white/90 dark:bg-[#0D0D12]/80 border-b border-stone-200 dark:border-white/10 overflow-x-auto select-none no-scrollbar">
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            {isCanvas ? (
              <FolderKanban className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-[#1831D7] dark:text-[#7F95FF]' : 'text-stone-400 dark:text-neutral-500'}`} />
            ) : isAudio ? (
              ...
              <FileText className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-[#1831D7] dark:text-[#7F95FF]' : 'text-stone-400 dark:text-neutral-500'}`} />
            )}
